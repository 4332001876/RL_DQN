...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,training_set):
        self.exp=training_set
        
 
    def __getitem__(self, index):
        
        return torch.tensor(self.exp[index][0],dtype=torch.float32), torch.tensor(self.exp[index][1],dtype=torch.float32)
...
def train(model, optimizer, loss_fn, train_loader, epochs=50):#pytorch神经网络训练函数
    ...
    for epoch in range(1, epochs+1):
        ...
        print('Epoch: {:d}, Training Loss: {:.4f}'.format(epoch, training_loss))#输出训练损失
...
class ExpBuffer:#经验池
    def __init__(self):    
        self.exp=collections.deque([],maxlen=MAX_EXP_SIZE)#双端队列，元素为字典exp:{"state":,"action":,"reward":,"state_next":,"is_done":}
    ...
    def get_anim_data(self,start_point):#从第start_point个数据点开始获取动画数据
        anim_data=[]
        i=start_point
...
def rand_exp_buffer(env,buffer,size):#随机构造经验池
    ...
    while count<size:  
        if env.win_fail==0:
            ...
            action=randseries[int(count/50)]
...
# Load and format data
    ...
    for i in range(60):
        for j in range(60):
            z[i][j]=torch.argmax(nn.forward(torch.tensor([x[i][j],y[i][j],0,0,10.0],dtype=torch.float32))).item()
    ...
    while count<size:  
        if env.win_fail==0:
            ...
            if random.random()>EPSILON:#epsilon_greedy
                action=torch.argmax(target_nn.forward(torch.tensor(state,dtype=torch.float32))).item()
            else:
...
for i in range(EPOCHS):#神经网络将训练EPOCHS个epoch
    training_set=[]
    ...
    for j in range(BATCH_SIZE*EPOCH_SIZE):
        ...
        if sample[j]["is_done"]==False:
            ...
            Qvalue=target_nn.forward(torch.tensor(next_state,dtype=torch.float32))#从目标网络获取各动作估值
            reward+=DISCOUNT_RATE*torch.max(Qvalue).item()#再由当前动作奖励更新估值
        result_est=nn.forward(torch.tensor(state,dtype=torch.float32))
        result_est[0][action]=reward-result_est[0][action]
        training_set.append([state,result_est])
    train_set=MyDataset(training_set)
    train_loader=torch.utils.data.DataLoader(train_set,batch_size=BATCH_SIZE)
    epoch_loss=train(nn,optimizer, F.mse_loss, train_loader,epochs=10)
    ...
    if i%3==2:
        net_file='DQN %03d.txt'%(i+1)
        torch.save(nn.state_dict(),net_file)   
...
#torch.save(nn.state_dict(),'DQN.txt')        
